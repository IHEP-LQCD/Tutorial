...
grep -r -C5 "str" param/
```
- `find` ：在 `param` 目录下搜索满足 `str` 条件的文件，显示搜索到的所有文件。若 `param` 为空，则默认为当前目录。
...
- `grep` ：在 `param` 文件中搜索关键词 `str` 所在行，显示搜索到的所有匹配行的内容。
  ...
  - `-r` ：若启用，则改为遍历 `param` 目录（递归地包括各个下级目录）中的所有文件，搜索关键词 `str` 所在行，显示搜索到的所有匹配行的内容及其所在文件路径。若 `param` 为空，则默认为当前目录。
